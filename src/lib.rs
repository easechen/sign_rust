...
impl LoginConfig {
    ...
    pub fn login(self, api: Api, client: &reqwest::blocking::Client) -> Result<reqwest::blocking::Response, reqwest::Error> {
        ...
        let res = client.post(if let Api::LoginApi(url) = api { url } else { panic!("Api type Error.") })
            .json(&form)
            .send();
...
pub fn get_course(login: String, api: Api, client: &reqwest::blocking::Client) -> Vec<Course> {
    ...
    let res = client.get(if let Api::CourseApi(url) = api { url } else { panic!("Error in User::new().") })
        .json(&form)
        .send().expect("Error: client.get.json.send.");
    ...
    for course in js["courses"].members() {
        ...
        let sign_info: Vec<SignInfo> = get_sign_info(Api::SignAttentionApi(sign_attention_api), &client);
        courses.push(
            Course::new(course["id"].to_string(), course["name"].to_string(), course["teacher"]["real_name"].to_string(), sign_info)
        )
    }
...
pub fn get_sign_info(api: Api, client: &reqwest::blocking::Client)  -> Vec<SignInfo>{
    let res= client.get(if let Api::SignAttentionApi(url) = api {url} else { panic!("Error in get_sign_info() client.get(url)") })
        .send()
...
impl User {
    ...
    pub fn print_course(&self) {
        ...
        for course in &self.course {
            println!("{}      {}    {}     {}", i, course.course_id, course.course_name, course.teacher);
        }
    }
}

impl Course {
    pub fn new(course_id: String, course_name: String, teacher: String, sign_info: Vec<SignInfo>) -> Course {
        Course {
            course_id,
            course_name,
            teacher,
    ...
    pub fn print_sign_code(&self) {
        ...
        for signItem in &self.sign_info {
            println!("{}      {}    {}    {}", i, signItem.time, signItem.code, signItem.statue);
            i += 1;
        }
    }
...
impl SignInfo {
    pub fn new(time: String, code: String, attendance_id: String, attendance_mode: String, statue: String) -> SignInfo {
        SignInfo {
            time,
            code,
            attendance_id,
    ...
    pub fn sign_post(&self, api: Api, client: &reqwest::blocking::Client) -> Result<reqwest::blocking::Response, reqwest::Error> {
        // creat form
        let mut form = HashMap::new();
        ...
        client.post(if let Api::SignPostApi(url) = api {url} else { panic!("Api type Error: expect SignPostApi.") })
            .json(&form)
            .send()
    }
}
